...
public class VehicleFollowing : MonoBehaviour {
	...
	void Update () {
		...
		//ifreacht the radius within the path then move to next point in the path
		if (Vector3.Distance(transform.position, targetPoint) < path.Radius) {
			//Don't move the vehicle if the path is finished
			if (curPathIndex < pathLength - 1) curPathIndex++;
			else if (isLooping) curPathIndex = 0;
			else return;
		}
		//Move vehicle until the end point is reached in the path
